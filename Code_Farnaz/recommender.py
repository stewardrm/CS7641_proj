...
class movie_lens_data():
    def __init__(self, movies, ratings, genome_scores,genome_tags,tags,truncate=True):
        ...
        # merge the movies dataframe with the ratings dataframe to get the average rating for each movie
        avg_ratings = self.ratings.groupby('movieId')['rating'].mean()
        self.movies = pd.merge(self.movies, avg_ratings, on='movieId', how='left')
        self.movies['rating'] = self.movies['rating'].fillna(0) # fill NaN values with 0
        self.movies['rating'] = self.movies['rating'].round()
        ...
        rates = self.movies['rating']
        generes=generes.str.replace('|', ' ')
        self.combined=list(self.titles+generes+' '+ rates.astype(str))
    ...
    def get_recommendations(self, selected_title, num_recommendations=5):
        ...
        movies=self.movies
        query_desc1 = movies[movies['title'] == selected_title]['genres'].values[0]
        query_desc2 = movies[movies['title'] == selected_title]['rating'].values[0]
        # tokenize and encode the query movie title and description with BERT
        query_input = self.bert_tokenizer([query_title + ' ' + query_desc1 + ' ' + str(query_desc2)], padding=True, truncation=True, return_tensors='pt')
        with torch.no_grad():
